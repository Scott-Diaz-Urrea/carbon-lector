...
export const EDFISICA_MODULES = [
  ...
  {id:'examenedfisica1', label:'Examen Final', open:true, key:'examenedfisica1'},
];
/* 4° nodo agregado (2026-08-09, "Examen Final") — las 3 posiciones
   ...
   Visuales/Música. */
export const EDFISICA_POS = [{x:70,y:86},{x:24,y:65},{x:70,y:43},{x:24,y:22}];
...
/* Niveles de dificultad (2026-08-09, mismo motor que el resto de 1°
   ...
   los 3 generadores `item.label` ya es la descripción/afirmación completa
   en texto — el emoji (aquí una animación `personActionSVG`) es
   decorativo, se saca en difícil sin dejar la pregunta sin sujeto. */
export function genMovimientoRound(nivel){
  ...
  const categorias = ['Locomoción','Manipulación','Estabilidad'];
  let opts;
  if(nivel==='facil'){
    const distract = pick(categorias.filter(function(c){ return c!==item.tipo; }));
    opts = shuffle([item.tipo, distract]).map(function(t){ return {label:t, value:t}; });
  }else{
    opts = shuffle(categorias).map(function(t){ return {label:t, value:t}; });
  }
  return {
    promptHTML: (showEmoji ? '<span class="prompt-emoji">'+item.emoji+'</span>' : '')+'<p class="prompt-hint">'+item.label+'. ¿Qué tipo de movimiento es?</p>',
...
export function genVidaActivaRound(nivel){
...
export function genSeguridadRound(nivel){
...
/* "Examen Final" (mismo patrón que el resto de 1° básico): mezcla los 3
   módulos de Educación Física 1° básico + los 3 niveles al azar. */
export function genExamenEdfisica1Round(){
  const gens = [genMovimientoRound, genVidaActivaRound, genSeguridadRound];
  const gen = pick(gens);
  const nivel = pick(['facil','normal','dificil']);
  return gen(nivel);
}
